...
import { homeHtml, disguisedHtml, maintenanceHtml } from './pages.js';
...
const config = {
  ...
  // 维护模式：设为 1/true/on/维护 时整站返回 503 维护页，留空或 0 为在线
  maintenance: /^(1|true|on|yes|维护)$/i.test((process.env.MAINTENANCE || '').trim())
};
...
export async function handle(request) {
  try {
    ...
    // robots.txt：全站禁止收录（优先于维护模式，保证爬虫任何时候都能读到）
    ...
    if (pathname === '/' || pathname === '') {
      ...
      if (config.disguiseUrl) {
        // 伪装页以真实 404 状态返回，与 nginx 行为一致，也不会被收录
